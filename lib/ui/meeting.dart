...
class MeetingPageState extends State<MeetingPage> with WidgetsBindingObserver {
  ...
  int _logShowLevel = 0;
  static const MethodChannel _tencentRTCffiChannel = MethodChannel('TencentRTCffi');
  ...
  _initRoom() async {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _initData();
      // 调用原生方法 enableVideoProcessByNative
      _enableVideoProcessByNative();
    });
  }

  // 调用原生方法 enableVideoProcessByNative
  Future<void> _enableVideoProcessByNative() async {
    try {
      await _tencentRTCffiChannel.invokeMethod('enableVideoProcessByNative', {"enable": true});
      _printLog(1, "enableVideoProcessByNative called successfully");
    } catch (e) {
      _printLog(1, "Error calling enableVideoProcessByNative: $e");
    }
  }
          ...
          child: TRTCCloudVideoView(
              ...
              onViewCreated: (viewId) async {
                if (item.userId == _meetModel.getUserInfo().userId) {
                  ...
                  _trtcCloud.startRemoteView(item.userId, item.type == 'video' ? TRTCVideoStreamType.big : TRTCVideoStreamType.sub, viewId);
                }
        ...
        child: new Container(
          ...
          child: new Row(
            ...
            children: <Widget>[
              IconButton(
                  ...
                  onPressed: () async {
                    if (_meetModel.getUserInfo().isSpeak) {
                      _txDeviceManager.setAudioRoute(TXAudioRoute.speakerPhone);
                    } else {
                      _txDeviceManager.setAudioRoute(TXAudioRoute.earpiece);
                    }
                    setState(() {
              ...
              Text(_meetModel.getMeetId().toString(), style: TextStyle(fontSize: 20, color: Colors.white)),
              TextButton(
                style: TextButton.styleFrom(backgroundColor: Colors.red, textStyle: const TextStyle(color: Colors.white)),
                onPressed: () async {
        ...
        child: new Container(
          ...
          child: new Row(
            ...
            children: <Widget>[
              ...
              IconButton(
                  icon: Icon(
                    _meetModel.getUserInfo().isOpenCamera ? Icons.videocam : Icons.videocam_off,
                    color: Colors.white,
                    size: 36.0,
                  ),
                  onPressed: () {
                        ...
                        (index) => LayoutBuilder(
                          ...
                          builder: (BuildContext context, BoxConstraints constraints) {
                            ...
                            return Container(
                              ...
                              child: Stack(
                                key: valueKey,
                                children: <Widget>[_renderView(item[index], valueKey, width, height), _videoVoice(item[index])],
                              ),
                            );
                          },
                        ),
